...
import { ArrowLeft, CheckCircle2, Lock, Server, Shield, UserPlus } from 'lucide-react'
        ...
        >
            ...
            <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-10 xl:px-14 min-h-0 pb-4 -mt-2">
                ...
                <p className="text-[13px] xl:text-[15px] leading-relaxed max-w-[380px] text-center mb-0" style={{ color: 'rgba(255,255,255,0.75)' }}>
                    Share who you are and why you need access. Primary Admins and Admins will be notified immediately.
                </p>
            </div>
            ...
            <div className="relative z-10 px-10 xl:px-14 pb-5 pt-3 shrink-0">
                <div className="flex items-stretch justify-center gap-3 mb-4">
                    {[
                        ...
                        { Icon: Lock, label: 'Approval Based', sub: 'Admin reviewed' },
                        { Icon: Server, label: 'Tracked Access', sub: 'Audit friendly' },
                    ].map((feat) => (
                        <div
                            key={feat.label}
...
export default function RequestInvitationPage() {
    ...
    const canSubmit = useMemo(
        () => fullName.trim().length >= 2 && email.trim().length > 0 && reason.trim().length >= 5,
    ...
    const handleSubmit = async (event: FormEvent) => {
        ...
        setError('')
        try {
            const response = await authAPI.requestAccess({
                full_name: fullName.trim(),
                email: email.trim(),
                reason: reason.trim(),
            })
            setMessage(response.data?.message || 'Your request has been submitted to the admin team.')
            setIsSubmitted(true)
        } catch (err: any) {
        ...
        <div className="min-h-screen flex">
            ...
            <div className="flex-1 flex items-center justify-center p-6 sm:p-10 bg-gray-50/80">
                <div className="w-full max-w-[480px] animate-fade-in">
                    <div className="mb-8">
                        <h2 className="text-[1.75rem] font-bold text-gray-900 mb-2 tracking-tight">Request an invitation</h2>
                        <p className="text-gray-500 text-[15px]">Tell us who you are and the admin team will review your access request.</p>
                    </div>
                    ...
                    {isSubmitted ? (
                        <div className="rounded-3xl border border-emerald-200 bg-white p-8 shadow-sm">
                            ...
                            <p className="mt-5 text-xs text-gray-400">Admins will receive an email notification and can grant access from Settings → Users.</p>
                            <div className="mt-6">
                                <BackToLogin />
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="rounded-3xl border border-gray-200 bg-white p-8 shadow-sm space-y-5">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Role / reason for access</label>
                                <textarea
                                    value={reason}
                                    onChange={(event) => setReason(event.target.value)}
                                    className="input-premium min-h-[120px] resize-none"
                                    placeholder="e.g. Sleep care coordinator managing incoming leads and follow-ups."
                                    required
                                />
                            </div>
